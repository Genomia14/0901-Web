...
// 추상 클래스 : 추상메서드를 포함하는 클래스는 추상클래스로 작성
// 일반 클래스에 추상메서드를 포함하면 에러가 발생
// 작성 방법 : class 선언 앞에 abstract를 붙이면 추상 클래스로 구현됨

// 반드시 필요한 메서드 이지만 각각 다른 코드가 실행되어야 하는 경우 추상메서드로 설정
// 추상클래스는 추상메서드가 포함되어있기 때문에 직접 인스턴화 하는것은 불가능함
// 반드시 상속하여 자식 클래스를 만들고 인스턴화 해야 함
abstract class Unit {
	String name;
	int hp;
...
// 추상 메서드 : 실행코드를 작성하지 않고 상속을 했을 때 실제 코드를
//						 자식클래스에서 각각 실행 코드를 구현하도록 만드는 메서드
