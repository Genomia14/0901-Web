...
public class Ex17_ArrayObjSort {
	...
	public static void main(String[] args) {
		...
		// 내림차순, 래퍼클래스로된 자료형만 실행가능
		System.out.println(Arrays.toString(intArr));

		System.out.println();

		// 문자열 정렬하기
		String[] strArr = { "kiwi", "apple", "melone", "banana" };
		System.out.println(Arrays.toString(strArr));
		Arrays.sort(strArr); // 사전순으로 오름차순
		System.out.println(Arrays.toString(strArr));
		Arrays.sort(strArr, Collections.reverseOrder()); // 사전순으로 내림차순
		System.out.println(Arrays.toString(strArr));

		System.out.println();

		// 글자 수 순으로 정렬하기
		String[] strArr2 = { "abc", "a", "abcde", "ab", "abcd" };
		System.out.println(Arrays.toString(strArr2));
		// 글자 수가 작은수에서 큰수로 오름차순 정렬
		Arrays.sort(strArr2, Comparator.comparingInt(String::length));
		System.out.println(Arrays.toString(strArr2));
		// 글자 수가 큰수에서 작은수로 오름차순 정렬
