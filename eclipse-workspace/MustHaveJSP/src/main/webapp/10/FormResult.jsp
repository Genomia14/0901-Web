...
<body>
	<h3>EL로 폼값 받기</h3>
	<ul>
		<!-- EL주의사항 : HTML, CSS, JS 코드 영역에서만 사용할 수 있음 -->
		...
		<!-- paramValues.key값 : 파라미터중 배열데이터를 가진 데이터를 출력 -->
		<% String[] interArr = request.getParameterValues("inter"); %>
		<li>관심사항 :
			${ paramValues.inter[0] } <%=interArr[0] %>
			${ paramValues.inter[1] } <%=interArr[1] %>
			${ paramValues.inter[2] } <%=interArr[2] %>
			${ paramValues.inter[3] } <%=request.getAttribute("interArr[3]") %> </li>
	</ul>
</body>
</html>