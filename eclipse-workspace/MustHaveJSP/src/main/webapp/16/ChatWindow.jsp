...
<title>웹소켓 채팅</title>
<script>
	// 웹 소켓 접속				ws://localhost:8080/MustHaveJSP
	...
	function sendMessage() {
		// 채팅창에 자신이 작성한 메시지를 추가
		chatWindow.innerHTML += "<div class='myMsg'>" + chatMessage.value + "</div>";
		// ChatServer.java 파일의 OnMessage를 실행
		webSocket.send(chatId + '|' + chatMessage.value);
		// 타이핑 내용 지우기
		chatMessage.value = "";
		// 채팅창의 스크롤 가장 아래로 내리는 코드
		// scrollTop : 상하로 스크롤의 위치를 설정하는 변수
	...
	// 웹소켓에서 에러가 발생했을때 실행되는 함수
	webSocket.onerror = function(event) {
		alert(event.data);
		chatWindow.innerHTML += "채팅 중 에러가 발생하였습니다.<br/>";
	};
//event : webSocket.send()가 실행될 때 메개변수로 사용된 데이터
	...
	// event : 대화명 | 메세지
		...
		if (content != "") {
			if (content.match("/")) {
				// 메시지 안에 있는ㄷ ㅐ화명이 자신의 이름이면 실행
				if (content.match(("/" + chatId))) {
					///*/대화명 */>					var temp = content.replace(("/" + chatId), "[귓속말] : ");
